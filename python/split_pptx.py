"""PowerPoint（.pptx）を Slide 単位で分割する。"""
...
SLIDE_RE = re.compile(r"<!--\s*Slide number:\s*\d+\s*-->", re.IGNORECASE)
...
def split_md(md_text: str, src_path: str, output_dir: str) -> list[str]:
    ...
    for i, m in enumerate(matches):
        ...
        target = out_dir / f"slide-{i + 1:02d}.md"
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 4:
        print("usage: split_pptx.py <md_file> <src_pptx> <out_dir>", file=sys.stderr)
        sys.exit(2)
