...
> **関連プロジェクト**: [[../README|フェーズインデックス]] | [[80_POC_Projects/POC_017_ClaudianBridge/02_設計文書/01_クラス設計|クラス設計]]
> **対応バージョン**: Claudian Bridge v0.41.0（基幹: v0.8.0・v0.16.0 `tts.inputAi` / v0.33〜0.41 TTS・outputsMirror 拡張を追記）
...
classDiagram
    ...
    class GeneralSection {
        ...
        +boolean outputsMirrorEnabled
        +string outputsMirrorPath
        +boolean hideDotFolders
    }
...
| `outputsMirrorEnabled` | `boolean` | `false` | 🆕 v0.41.0: Outputs フォルダミラリングの有効化（NTFS ジャンクション）。Vault 内 `Outputs` 実フォルダ既存時はトグル無効化で ON 不可 |
| `outputsMirrorPath` | `string` | `Documents\ObsidainOutputs`（`app.getPath('documents')` 動的解決） | 🆕 v0.41.0: ミラー元の外部実体フォルダ。設定画面の「📂 開く」ボタンで `shell.openPath`（不在時は自動作成） |
| `hideDotFolders` | `boolean` | `true` | 🆕 v0.41.0: 「`.` で始まるフォルダを非表示」。Whitelist CSS に dot フォルダ非表示ルールを追加 |
...
| `popupPosition` | 🆕 v0.38.0: `'top-right' \| 'bottom'` | `'top-right'` | 選択ポップアップの表示位置。未設定・不正値は `'top-right'` に正規化 |
...
**🆕 v0.27.0〜v0.37.2 の TTS 追加フィールド（抜粋）**:

| フィールド | 型 / 既定 | 説明 |
|-----------|-----------|------|
| `tts.mdReadHighlight` | `{ enabled: true, highlightColor: string }` | v0.33.0: MD 読み上げ位置ハイライト |
| `tts.autoRead` | `scope: 'header' \| 'full'` 等 | 自動読み上げ範囲（v0.13.0〜） |
| `tts.autoReadReportScript` | `boolean` / `true` | v0.28.0: 完了報告を読上げ用スクリプトに整形 |
| `tts.excludeCallouts` | `boolean` / `true` | v0.15.0: コールアウト除外 |
| `addToTtsLanguageMode` / `autoReadLanguageMode` | `'auto' \| 'ja' \| 'zh' \| 'en'` | v0.27.0: 経路別言語モード |
| `tts.edgeCloud` | `{ serverUrl, authToken, timeout }` | v0.27.0: クラウド EdgeTTS HTTPS プロキシ |
| `tts.listenerProfile` | `'original' \| 'workplace' \| 'customer' \| 'family' \| 'classroom' \| 'boss' \| 'dr'` | v0.36.0: 聴き手プロファイル（既定 `original`） |
| `tts.termsDict` | `string`（Vault 内 MD パス） | v0.36.0: 用語 → やさしい表現の対照表 |
| `tts.llmRewriteCache` | `boolean` / `true` | v0.37.0: LLM 原稿キャッシュ（100 件 LRU） |
| `tts.llmRewriteConcurrency` | `number` / `2` | v0.37.1: LLM 並列生成数（1〜8 にクランプ） |
...
```json
{
  "general": {
    ...
    "quotaRefreshSec": 60,
    "quotaSwitchSec": 5,
    "outputsMirrorEnabled": false,
    "outputsMirrorPath": "C:\\Users\\superlambkin\\Documents\\ObsidainOutputs",
    "hideDotFolders": true
  },
  "quota": {
...
| バージョン | 日付 | 機能改良内容 |
|-----------|:----:|-------------|
| v0.33.0 | 2026-09-02 | `tts.mdReadHighlight: { enabled, highlightColor }` 新設（MD 読み上げ位置ハイライト） |
| v0.34.0 | 2026-09-04 | `general.mermaidRender`（既定 ON）新設 — チャット内 Mermaid 自動描画 |
| v0.36.0 | 2026-09-05 | `tts` に聴き手プロファイル（7 種）+ `tts.termsDict` 追加 |
| v0.37.0/0.37.1 | 2026-09-05 | `tts.llmRewriteCache` / `tts.llmRewriteConcurrency`（1〜8・既定 2）新設 |
| v0.38.0 | 2026-09-07 | `selection.popupPosition`（`'top-right'` 既定 / `'bottom'`）新設 |
| v0.41.0 | 2026-09-13 | `general.outputsMirrorEnabled`（既定 OFF）/ `general.outputsMirrorPath`（Documents\ObsidainOutputs 動的解決）/ `general.hideDotFolders`（既定 ON）新設 |
...
| 3.1.0 | 2026-08-16 | 実コード v0.16.0 同期（`tts.inputAi` 追記） | MiuMiu 🐾 |
| 3.2.0 | 2026-09-13 | v0.41.0 準拠更新（改定履歴セクション新設 + `general.outputsMirror*` / `general.hideDotFolders` / `selection.popupPosition` / TTS 拡張フィールド追記） | MiuMiu 🐾 |

---

*🗄️ データモデル v3.2.0 · Claudian Bridge v0.41.0 · MiuMiu 🐾*
