...
## 🪞 11. ジャンクション作成・削除の安全性（v0.41.0 新設）

**ソース**: `src/features/outputs-mirror/manager.ts`（Outputs フォルダミラリング）
...
```mermaid
flowchart TB
    A["apply(enabled, externalPath)"] --> B{"enabled?"}
    B -- "いいえ" --> C{"Vault/Outputs 存在?"}
    C -- "はい" --> D{"lstat → isSymbolicLink?"}
    D -- "はい" --> E["rmdirSync でリンクのみ削除"]
    D -- "いいえ" --> F["削除しない → vault_exists<br/>（実データ保護・最重要）"]
    C -- "いいえ" --> G["inactive"]
    B -- "はい" --> H{"Vault/Outputs 実フォルダ?"}
    H -- "はい" --> I["fs 操作なし → vault_exists<br/>（既存実フォルダ優先）"]
    H -- "いいえ" --> J["外部不在なら mkdirSync →<br/>symlinkSync(外部, Vault, 'junction')"]
```
...
| 脅威 | 防御 | ソース |
|------|------|--------|
| 削除対象が実フォルダ（ユーザーデータ）なのに削除 | **`lstatSync` + `isSymbolicLink()` 必須判定** — ジャンクション以外は削除せず `vault_exists` を返す | `outputs-mirror/manager.ts` |
| 既存の Vault 内 `Outputs` 実フォルダをジャンクションで上書き | 実フォルダ存在時は fs 操作を一切行わず有効化不可（設定 UI トグル無効化 + 理由表示） | 同上 + `SettingTabWhitelist.ts` |
| 外部パスの誤設定・作成失敗 | `mkdirSync` 失敗 / `symlinkSync` 例外を捕捉し Notice + `status: "error"`（Fail-soft・プラグイン継続） | 同上 |
| Windows 以外で symlink 型 `junction` を指定 | `process.platform === 'win32'` 判定で機能無効化・設定非表示 | 同上 |
| ミラー先経由での Vault 外データ露出 | ジャンクションは 1 方向（Vault/Outputs → Documents/ObsidainOutputs）のみ・chroma_db と同一方式の実績あり | 同上 |
| 自己更新の改ざん配布物 | v0.32.10: DL 前に `.backup/<UTC-ISO>/` へ退避し、失敗時はバックアップから復旧可能 | `features/self-update/backup-manager.ts` |
...
- [x] ジャンクション削除は `lstat` 判定必須（実フォルダ絶対削除禁止・v0.41.0）
- [x] 既存実フォルダ優先（Outputs 実フォルダ既存時は fs 操作なし・トグル無効化）
...
- [[80_POC_Projects/POC_017_ClaudianBridge/02_設計文書/05_API設計|API 設計]]
...
| バージョン | 日付 | 機能改良内容 |
|-----------|:----:|-------------|
| v0.32.9〜0.32.10 | 2026-09-03/04 | ハイライト anchor の正規化マッチング（記号フィルタ差異の恒久解消）・自己更新に `.backup/<UTC-ISO>/` 退避を追加 |
| v0.33.2 | 2026-09-02 | MD 読み上げハイライトの DOM 配線漏れ修正（state 単体テストでは検出できない統合層バグ） |
| v0.39.0/v0.40.0 | 2026-09-11 | Think モード用プロバイダ API キーの取扱いを既存 `quota.*` キー管理方針に統合 |
| v0.41.0 | 2026-09-13 | **ジャンクション作成・削除の安全性**を §11 に新設（`lstat` による実フォルダ保護・実フォルダ優先・Fail-soft エラー通知） |
...
| v4.1.0 | 2026-09-13 | v0.41.0 準拠更新（改定履歴セクション新設 + §11「ジャンクション作成・削除の安全性」追加：lstat による実フォルダ保護・チェックリスト 2 項目追記） | MiuMiu 🐾 |

---

*🔒 セキュリティ設計 v4.1.0 · claudian-bridge 0.41.0 実コード準拠 · MiuMiu 🐾*